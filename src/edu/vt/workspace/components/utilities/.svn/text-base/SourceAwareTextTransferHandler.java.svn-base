...
/**
 * This class is here to help create a way to get the source location for a drag event. It is
 *  an enormous and ungainly hack. This is copied completely from the TransferHandler that
 *  is found inside of BasicTextUI (as a private inner class). The only changes were to use my
 * Transferable (which is also mostly copied). This is the only way that I could get the functionality
 * that I wanted while retaining all of the basic text transfer functionaity.
 *
 * @author cpa
 */
public class SourceAwareTextTransferHandler extends TransferHandler implements UIResource {
    ...
    protected Transferable createTransferable(JComponent comp) {
        ...
        return (Transferable) ((p0 != p1) ? (new SourceAwareTextTransferable(exportComp, p0, p1)) : null);
    }
    ...
    protected void exportDone(JComponent source, Transferable data, int action) {
        ...
        if (shouldRemove && action == MOVE) {
            SourceAwareTextTransferable t = (SourceAwareTextTransferable) data;
            t.removeText();
        }
